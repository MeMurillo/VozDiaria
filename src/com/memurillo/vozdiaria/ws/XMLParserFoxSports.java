...
public class XMLParserFoxSports implements XMLParserInterface {
	// We don't use namespaces
    private static final String ns = null;
    private int cantNoticiasConf;
    
    public XMLParserFoxSports(String cantidadNoticiasConfiguradas){
    ...
    private Noticia readEntry(XmlPullParser parser) throws XmlPullParserException, IOException {
    	...
    	while (parser.next() != XmlPullParser.END_TAG) {
    		...
    		if (name.equals("title")) {
    			...
    			link = Util.reemplazarCaracteresHtml(link);
    		}
    		else {
    			skip(parser);
    		}
    	}
    	return new Noticia(title, link, descripcion, Constantes.FOXSPORTS);
    }
